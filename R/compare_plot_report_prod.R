...
create_ge_compare_plot_report_prod <- function(pn_cur_ge_label,
  ...
  if (pb_debug) {
    if (is.null(plogger)){
      lgr <- get_qgert_logger(ps_logfile = 'create_ge_compare_plot_report_prod.log', ps_level = 'INFO')
    } else {
      lgr <- plogger
    }
    qgert_log_info(plogger = lgr, ps_caller = 'create_ge_plot_report',
                   ps_msg = " Start of function create_ge_compare_plot_report_prod ... ")
    qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_prod",
