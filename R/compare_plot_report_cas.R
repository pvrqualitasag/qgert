...
###
###
###   Purpose:   Create a Comparison Plot Report for CAS
###   started:   2019-10-14 (pvr)
###
### ########################################################## ###


## -- Creator Function for CAS ----------------------------------------- ##

#' @title Comparison Plot Report Creator Function For Aufzuchtverlust (CAS)
#'
#' @description
...
#' }
#'
#' @export create_ge_compare_plot_report_cas
create_ge_compare_plot_report_cas <- function(pn_cur_ge_label,
                                              pn_prev_ge_label,
  ...
  if (pb_debug) {
    if (is.null(plogger)){
      lgr <- get_qgert_logger(ps_logfile = 'create_ge_compare_plot_report_cas.log', ps_level = 'INFO')
    } else {
      lgr <- plogger
    }
    qgert_log_info(plogger = lgr, ps_caller = 'create_ge_plot_report',
                   ps_msg = " Start of function create_ge_compare_plot_report_cas ... ")
    qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_cas",
  ...
  if (is.null(l_plot_opts)){
    l_plot_opts <- get_default_plot_opts_cas()
  }
  ...
  for (breed in vec_breed){
    ...
    for (sex in l_plot_opts$vec_sex){
      ...
      s_ge_dir <- file.path(s_cur_plot_root, breed, paste0("YearMinus0/compare", sex))
      if (pb_debug)
        qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_cas",
      ...
      s_report_text  <- glue::glue('## Comparison Of Plots\n',
                                   'Plots compare estimates of Aufzuchtverlust (cas) for {tolower(sex)}',
                                   ' of breed {breed}',
      ...
      s_rep_path <- file.path(s_ge_dir, paste0('ge_plot_report_cas_compare', sex, '_', breed, '.Rmd', collapse = ''))
      if (pb_debug)
        qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_cas",
  ...
  if (pb_debug)
    qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_cas",
             ps_msg    = " * End of function create_ge_compare_plot_report_cas")
