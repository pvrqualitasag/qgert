...
## -- Creator Function for BIRTH ----------------------------------------- ##

#' @title Comparison Plot Report Creator Function For Production (BIRTH)
#'
#' @description
...
#' @examples
#' \dontrun{
#' create_ge_compare_plot_report_birth(pn_cur_ge_label  = 2008,
#'                                  pn_prev_ge_label = 2004,
#'                                  pb_debug = TRUE)
#' }
#'
#' @export create_ge_compare_plot_report_birth
create_ge_compare_plot_report_birth <- function(pn_cur_ge_label,
                                              pn_prev_ge_label,
  ...
  if (pb_debug) {
    if (is.null(plogger)){
      lgr <- get_qgert_logger(ps_logfile = 'create_ge_compare_plot_report_birth.log', ps_level = 'INFO')
    } else {
      lgr <- plogger
    }
    qgert_log_info(plogger = lgr, ps_caller = 'create_ge_plot_report',
                   ps_msg = " Start of function create_ge_compare_plot_report_birth ... ")
    qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_birth",
  ...
  if (is.null(l_plot_opts)){
    l_plot_opts <- get_default_plot_opts_birth()
  }
  ...
  s_rep_path <- file.path(s_ge_dir, paste0('ge_plot_report_birth_compare.Rmd', collapse = ''))
  if (pb_debug)
    qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_birth",
  ...
  if (pb_debug)
    qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_birth",
             ps_msg    = " * End of function create_ge_compare_plot_report_birth")
