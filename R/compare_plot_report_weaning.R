...
## -- Creator Function for WEANING ----------------------------------------- ##

#' @title Comparison Plot Report Creator Function For Production (WEANING)
#'
#' @description
...
#' @examples
#' \dontrun{
#' create_ge_compare_plot_report_weaning(pn_cur_ge_label  = 2008,
#'                                  pn_prev_ge_label = 2004,
#'                                  pb_debug = TRUE)
#' }
#'
#' @export create_ge_compare_plot_report_weaning
create_ge_compare_plot_report_weaning <- function(pn_cur_ge_label,
                                              pn_prev_ge_label,
  ...
  if (pb_debug) {
    if (is.null(plogger)){
      lgr <- get_qgert_logger(ps_logfile = 'create_ge_compare_plot_report_weaning.log', ps_level = 'INFO')
    } else {
      lgr <- plogger
    }
    qgert_log_info(plogger = lgr, ps_caller = 'create_ge_plot_report',
                   ps_msg = " Start of function create_ge_compare_plot_report_weaning ... ")
    qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_weaning",
  ...
  if (is.null(l_plot_opts)){
    l_plot_opts <- get_default_plot_opts_weaning()
  }
  ...
  s_rep_path <- file.path(s_ge_dir, paste0('ge_plot_report_weaning_compare.Rmd', collapse = ''))
  if (pb_debug)
    qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_weaning",
  ...
  if (pb_debug)
    qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_weaning",
             ps_msg    = " * End of function create_ge_compare_plot_report_weaning")
