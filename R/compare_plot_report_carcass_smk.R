...
## -- Creator Function for CARCASS SMK ----------------------------------------- ##

#' @title Comparison Plot Report Creator Function For Production (CARCASS SMK)
#'
#' @description
...
#' @examples
#' \dontrun{
#' create_ge_compare_plot_report_carcass_smk(pn_cur_ge_label  = 2008,
#'                                  pn_prev_ge_label = 2004,
#'                                  pb_debug = TRUE)
#' }
#'
#' @export create_ge_compare_plot_report_carcass_smk
create_ge_compare_plot_report_carcass_smk <- function(pn_cur_ge_label,
                                              pn_prev_ge_label,
  ...
  if (pb_debug) {
    if (is.null(plogger)){
      lgr <- get_qgert_logger(ps_logfile = 'create_ge_compare_plot_report_carcass_smk.log', ps_level = 'INFO')
    } else {
      lgr <- plogger
    }
    qgert_log_info(plogger = lgr, ps_caller = 'create_ge_plot_report',
                   ps_msg = " Start of function create_ge_compare_plot_report_carcass_smk ... ")
    qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_carcass_smk",
  ...
  if (pb_debug)
    qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_carcass_smk",
             ps_msg    = " * End of function create_ge_compare_plot_report_carcass_smk")
