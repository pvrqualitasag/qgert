...
create_ge_compare_plot_report_mar <- function(pn_cur_ge_label,
  ...
  if (pb_debug) {
    if (is.null(plogger)){
      lgr <- get_qgert_logger(ps_logfile = 'create_ge_compare_plot_report_mar.log', ps_level = 'INFO')
    } else {
      lgr <- plogger
    }
    qgert_log_info(plogger = lgr, ps_caller = 'create_ge_plot_report',
                   ps_msg = " Start of function create_ge_compare_plot_report_mar ... ")
    qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_mar",
  ...
  for (breed in vec_breed){
    ...
    for (sex in l_plot_opts$vec_sex){
     ...
     # put together all directory names, start with GE working directory
      s_ge_dir <- file.path(s_cur_plot_root, breed, paste0("zws/compare", sex, collapse = ""))
      if (pb_debug)
        qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_mar",
                 ps_msg    = paste0(" ** GE workdir: ", s_ge_dir, collapse = ""))
      # archive directory
      s_arch_dir <- file.path(s_prev_plot_root, breed, paste0("zws/compare", sex, collapse = ""))
    if (pb_debug)
      qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_mar",
