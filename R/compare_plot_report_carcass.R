...
###   started:   2020-09-29 (skn)
###
### ########################################################## ###


## -- Creator Function for CARCASS ----------------------------------------- ##

#' @title Comparison Plot Report Creator Function For Production (CARCASS)
#'
#' @description
...
#' @examples
#' \dontrun{
#' create_ge_compare_plot_report_carcass(pn_cur_ge_label  = 2008,
#'                                  pn_prev_ge_label = 2004,
#'                                  pb_debug = TRUE)
#' }
#'
#' @export create_ge_compare_plot_report_carcass
create_ge_compare_plot_report_carcass <- function(pn_cur_ge_label,
                                              pn_prev_ge_label,
  ...
  if (pb_debug) {
    if (is.null(plogger)){
      lgr <- get_qgert_logger(ps_logfile = 'create_ge_compare_plot_report_carcass.log', ps_level = 'INFO')
    } else {
      lgr <- plogger
    }
    qgert_log_info(plogger = lgr, ps_caller = 'create_ge_plot_report',
                   ps_msg = " Start of function create_ge_compare_plot_report_carcass ... ")
    qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_carcass",
  ...
  if (is.null(l_plot_opts)){
    l_plot_opts <- get_default_plot_opts_carcass()
  }
  ...
  if (pb_debug)
    qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_carcass",
             ps_msg    = " * End of function create_ge_compare_plot_report_carcass")
