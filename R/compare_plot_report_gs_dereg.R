...
create_ge_compare_plot_report_gs_dereg <- function(pn_cur_ge_label,
  ...
  for (breed in l_plot_opts$vec_breed){
    ...
    for (comp in l_plot_opts$vec_comparisons){
      ...
      s_ge_dir <- file.path(l_plot_opts$ge_dir_stem, breed, "plotDereg", comp)
      if (pb_debug)
        qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_gs_dereg",
                 ps_msg    = paste0(" ** GE workdir: ", s_ge_dir, collapse = ""))
      # archive directory
      s_arch_dir <- file.path(l_plot_opts$arch_dir_stem, pn_prev_ge_label,
                              "gs/work", breed, "plotDereg", comp)
      if (pb_debug)
        qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_gs_dereg",
