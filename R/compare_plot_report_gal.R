...
#' @title Comparison Plot Report Creator Function For Fruchtbarkeit (GAL)
#'
#' @description
...
create_ge_compare_plot_report_gal <- function(pn_cur_ge_label,
  ...
  for (breed in l_plot_opts$vec_breed){
    ...
    for (comp in l_plot_opts$vec_comparisons){
      ...
      s_ge_dir <- file.path(l_plot_opts$ge_dir_stem, breed, paste0("ym0/compare", comp))
      if (pb_debug)
        qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_gal",
