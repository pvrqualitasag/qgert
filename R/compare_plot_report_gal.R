...
###
###
###   Purpose:   Create a Comparison Plot Report for GAL
###   started:   2019-10-14 (pvr)
###
### ########################################################## ###


## -- Creator Function for GAL ----------------------------------------- ##

#' @title Comparison Plot Report Creator Function For Geburtsablauf (GAL)
#'
#' @description
...
#' @examples
#' \dontrun{
#' create_ge_compare_plot_report_gal(pn_cur_ge_label  = 1908,
#'                                   pn_prev_ge_label = 1904,
#'                                   pb_debug = TRUE)
#' }
#'
#' @export create_ge_compare_plot_report_gal
create_ge_compare_plot_report_gal <- function(pn_cur_ge_label,
  ...
  if (pb_debug) {
    if (is.null(plogger)){
      lgr <- get_qgert_logger(ps_logfile = 'create_ge_compare_plot_report_gal.log', ps_level = 'INFO')
    } else {
      lgr <- plogger
    }
    qgert_log_info(plogger = lgr, ps_caller = 'create_ge_plot_report',
                   ps_msg = " Start of function create_ge_compare_plot_report_gal ... ")
  ...
  for (breed in vec_breed){
    ...
    for (comp in l_plot_opts$vec_comparisons){
      ...
      if ("gebv_subdir" %in% names(l_plot_opts)){
        s_ge_dir <- file.path(l_plot_opts$ge_dir_stem, breed, l_plot_opts$gebv_subdir, paste0("compare", comp))
      } else {
        s_ge_dir <- file.path(l_plot_opts$ge_dir_stem, breed, paste0("compare", comp))
      }
      if (pb_debug)
        qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_gal",
  ...
  if (pb_debug)
    qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_gal",
             ps_msg    = " * End of function create_ge_compare_plot_report_gal")
