...
###   started:   2020-09-28 (skn)
###
### ########################################################## ###


## -- Creator Function for AG ----------------------------------------- ##

#' @title Comparison Plot Report Creator Function For Production (AG)
#'
#' @description
...
#' @examples
#' \dontrun{
#' create_ge_compare_plot_report_ag(pn_cur_ge_label  = 2004,
#'                                  pn_prev_ge_label = 2008,
#'                                  pb_debug = TRUE)
#' }
#'
#' @export create_ge_compare_plot_report_ag
create_ge_compare_plot_report_ag <- function(pn_cur_ge_label,
                                              pn_prev_ge_label,
  ...
  if (pb_debug) {
    if (is.null(plogger)){
      lgr <- get_qgert_logger(ps_logfile = 'create_ge_compare_plot_report_ag.log', ps_level = 'INFO')
    } else {
      lgr <- plogger
    }
    qgert_log_info(plogger = lgr, ps_caller = 'create_ge_plot_report',
                   ps_msg = " Start of function create_ge_compare_plot_report_ag ... ")
    qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_ag",
  ...
  if (is.null(l_plot_opts)){
    l_plot_opts <- get_default_plot_opts_ag()
  }
  ...
  for (breed in vec_breed){
    ...
    for (sex in l_plot_opts$vec_sex){
      ...
      s_ge_dir <- file.path(s_cur_plot_root, breed, paste0("compare_", tolower(sex), collapse = ""))
      if (pb_debug)
        qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_ag",
                 ps_msg    = paste0(" ** GE workdir: ", s_ge_dir, collapse = ""))
      # archive directory
      s_arch_dir <- file.path(s_prev_plot_root, breed,paste0("compare_", tolower(sex), collapse = ""))
      if (pb_debug)
        qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_ag",
  ...
  if (pb_debug)
    qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_ag",
             ps_msg    = " * End of function create_ge_compare_plot_report_ag")
