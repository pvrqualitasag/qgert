...
## Disclaimer
The definition of a `comparison plot report` and how such a report is generated is described in this vignette.
...
tbl_plot_nrk %>%
  ...
  geom_text(aes(x, y, label = paste0('Correlation: ', round(n_cor_current_prev, digits = 3))),
                data = data.frame(x = xrng[2], y = yrng[1]), hjust = 1, size = 3)

  # geom_text(aes(x, y, label = c(paste0('SD: ', round(n_sd_current, digits = 1)),
  #                               paste0('Mean: ', round(n_mean_current, digits = 1)),
  #                                   paste0('Correlation: ', round(n_cor_current_prev, digits = 3))),
  #           data = data.frame(x = rep(xrng[2], 3), y = yrng[1] + 2*(0:2)), hjust = 1, size = 2))
...
The above shown comparison plots are done during each evaluation. The plots by themselves are only meaningful with a lot of expertise and experience. But when comparing the same plot to the version that was produced during the previous evaluation, then this could be meaningful.

The comparison plot report is a document that contains a series of plots which are arranged in two columns. The plots next to each other are the corresponding plots from the different evaluation runs. One of the plots was taken from the previous evaluation run. The second plot is a comparison plot from the current evaluation run. Given the fact that the plot from the previous run was considered to be satisfactory, the new comparison plot can be judged based on the older plot. The judgement is mainly based on the spread of the points around the $y=x$ line and based on the correlation values presented in the comparison plot. 
